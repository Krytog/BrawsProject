...
namespace {
    Position QuarterPosition(uint8_t x, uint8_t y) {
        const double coef_x = 0.125 * x - 0.375;
        const double coef_y = 0.125 * (3 - y) - 0.375;
    ...
    void CreateQuarter(uint8_t x, uint8_t y, std::string_view path) {
        ...
        CreateGameObject<PersistentVisibleObject>(new Position(pos), nullptr,
                                                  new BasicSprite(path, pos, MAP_WIDTH / 4, MAP_HEIGHT / 4, 0, IVisibleObject::RenderLayers::BackGround),
                                                  TAGS_MAPS_Ludus);
    }
}
