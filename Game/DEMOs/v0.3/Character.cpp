...
Character::Character(const CharacterInitArgPack& arg_pack)
    : moving_direction_(RIGHT),
      health_(arg_pack.start_health),
      shoot_pos_(arg_pack.shoot_pos),
    ...
    engine_->CreateEvent(&Character::ReadyToDestroy, std::tuple(this), engine_, &Engine::Destroy,
                         std::tuple(this), EventStatus::Disposable);
}
