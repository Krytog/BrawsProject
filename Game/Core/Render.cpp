...
public:
    RenderImplementation(): window_(sf::RenderWindow(sf::VideoMode(kWindowWidth, kWindowHeight), kWindowTitle)) {
    ...
    void RenderAll() {
        if (!window_.isOpen()) {
            // надо как-то прервать весь игровой цикл как-то видимо, но это потом подумаем
            return;
        }
        ...
        drawable_canvas.setPosition(0, 0);
        drawable_canvas.setScale(window_.getSize().x / drawable_canvas.getLocalBounds().width, window_.getSize().y / drawable_canvas.getLocalBounds().height);
