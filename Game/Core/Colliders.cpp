...
bool CheckImplementation<RectangleCollider, RectangleCollider>(
  ...
  const double x_distance = first->position_.GetCoordinates().first -
                             second->position_.GetCoordinates().first;
  const double y_distance = first->position_.GetCoordinates().second -
...
std::optional<Position> IntersectionImplementation<CircleCollider, RectangleCollider>(
    ...
    const double first_intersection_distance =
            (pair.first->GetCoordinatesAsVector2D() -
             rectangle->position_.GetCoordinatesAsVector2D())
                    .Length();
    const double second_intersection_distance =
            (pair.second->GetCoordinatesAsVector2D() -
             rectangle->position_.GetCoordinatesAsVector2D())
                    .Length();
    const Position closest_point =
            first_intersection_distance <= second_intersection_distance
    ...
    const double y_distance_final =
            std::abs(rectangle->position_.GetCoordinates().second -
                     closest_point.GetCoordinates().second);
    if (x_distance_final <= rectangle->width_ / 2 &&
           y_distance_final <= rectangle->height_ / 2) {
...
std::optional<Position> IntersectionImplementation<RectangleCollider, CircleCollider>(
        const RectangleCollider *rectangle, const CircleCollider *circle) {
    return IntersectionImplementation<CircleCollider, RectangleCollider>(circle,
                                                                  rectangle);
}
...
std::optional<Position> IntersectionImplementation<RectangleCollider, RectangleCollider>(
    ...
    first_rect[0] = Vector2D(first_center.GetCoordinates().first - first->width_ / 2, first_center.GetCoordinates().second - first->height_ / 2);
    first_rect[1] = Vector2D(first_center.GetCoordinates().first - first->width_ / 2, first_center.GetCoordinates().second + first->height_ / 2);
    first_rect[2] = Vector2D(first_center.GetCoordinates().first + first->width_ / 2, first_center.GetCoordinates().second + first->height_ / 2);
    first_rect[3] = Vector2D(first_center.GetCoordinates().first + first->width_ / 2, first_center.GetCoordinates().second - first->height_ / 2);

    second_rect[0] = Vector2D(second_center.GetCoordinates().first - second->width_ / 2, second_center.GetCoordinates().second - second->height_ / 2);
    second_rect[1] = Vector2D(second_center.GetCoordinates().first - second->width_ / 2, second_center.GetCoordinates().second + second->height_ / 2);
    second_rect[2] = Vector2D(second_center.GetCoordinates().first + second->width_ / 2, second_center.GetCoordinates().second + second->height_ / 2);
    second_rect[3] = Vector2D(second_center.GetCoordinates().first + second->width_ / 2, second_center.GetCoordinates().second - second->height_ / 2);
    ...
    auto checker = [](const Vector2D& vertex, const RectangleCollider* ptr){
        ...
        return x_dist <= (width + width) / 2 &&
               y_dist <= (height + height) / 2;
    };
