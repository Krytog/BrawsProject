...
Canvas::Image::Image(const Position *pos, const double &width,
                     const double &height)
...
void Canvas::Image::Clear() { image_.reset(); }
...
void Canvas::Image::ResizeImage(const sf::Image &originalImage,
...
void Canvas::Draw(const Canvas::Image *image) {
  ...
  canvas_->copy(*(image->image_),
                kFrameWidth + image_pos.first,
                kWindowHeight + kFrameHeight - image->height_ - image_pos.second);
}

// Тут пока написана хуйня, но это потом
void Canvas::Clear() { canvas_.reset(); }
