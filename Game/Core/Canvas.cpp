...
void Canvas::Image::LoadFromStaticImage(const StaticImage& static_image, const Position& orig_image_pos,
                                        const Position& rectangle_left_up_corner,
                                        const Position& rectangle_right_down_corner, const size_t& orig_width,
    ...
    frame_image.copy(static_image.GetEntireImage(), 0, 0,
                     sf::IntRect(left_up_coords.first, left_up_coords.second, right_down_coords.first,
                                 right_down_coords.second));
...
void Canvas::Draw(Canvas::Image* image) {
    ...
    const Vector2D offset = global_pos.GetCoordinatesAsVector2D() - global_center_.GetCoordinatesAsVector2D();
    const long long local_pos_x = LOCAL_CENTER_X + offset.GetCoordinates().first - (image->width_ / 2);
    const long long local_pos_y = LOCAL_CENTER_Y - offset.GetCoordinates().second - (image->height_ / 2);
