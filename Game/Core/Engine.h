...
public:
    ...
    //temporary public, should be private in the future as it will be called in Update method of Engine;
    ...
    void Invoke(const std::chrono::milliseconds& milliseconds, void(*func)(Args...), Args... args) {
    ...
    void Invoke(const std::chrono::milliseconds& milliseconds, F* pointer, void(F::*func)(Args...), Args... args) {
        delay_queue_.PushTime(std::chrono::steady_clock::now() + milliseconds, pointer, func, args...);
    }
    template<typename F, typename... Args>
    ...
    template<typename F, typename... Args>
    void Invoke(const std::chrono::milliseconds& milliseconds, const F* pointer, void(F::*func)(Args...) const, Args... args) {
        delay_queue_.PushTime(std::chrono::steady_clock::now() + milliseconds, pointer, func, args...);
    }
    template<typename F, typename... Args>
    void Invoke(const uint64_t ticks_count, const F* pointer, void(F::*func)(Args...) const, Args... args) {
        delay_queue_.PushTicks(ticks_count, pointer, func, args...);
    }
...
public:    uint64_t ticks_count_ = 0;
};
