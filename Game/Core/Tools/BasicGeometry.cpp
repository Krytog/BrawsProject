...
bool IsBelongToSegment(const Position &point, const Position &seg_a,
                       const Position &seg_b) {
  Vector2D ray1 =
      seg_a.GetCoordinatesAsVector2D() - seg_b.GetCoordinatesAsVector2D();
  Vector2D ray2 =
      seg_b.GetCoordinatesAsVector2D() - seg_a.GetCoordinatesAsVector2D();
  Vector2D pp1 =
      point.GetCoordinatesAsVector2D() - seg_b.GetCoordinatesAsVector2D();
  Vector2D pp2 =
      point.GetCoordinatesAsVector2D() - seg_a.GetCoordinatesAsVector2D();
  if (std::abs(ray1 * pp1 - ray1.Length() * pp1.Length()) <
          PRECISION &&
      std::abs(ray2 * pp2 - ray2.Length() * pp2.Length()) <
          PRECISION) {
    return true;
  }
  return false;
...
double Line::operator()(const Position &point) const {
  return A_ * point.GetCoordinates().first +
         B_ * point.GetCoordinates().second + C_;
}
...
bool DoesIntersect(const Position &b1, const Position &e1, const Position &b2,
                   const Position &e2) {
...
Circle::Intersect(const Line &line) const {
  ...
  double x_ref =
      -nline.A_ * nline.C_ / (nline.A_ * nline.A_ + nline.B_ * nline.B_);
  double y_ref =
      -nline.B_ * nline.C_ / (nline.A_ * nline.A_ + nline.B_ * nline.B_);

  if (nline.C_ * nline.C_ -
    ...
    double det =
        radius_ * radius_ -
        nline.C_ * nline.C_ / (nline.A_ * nline.A_ + nline.B_ * nline.B_);
    double coef = std::sqrt(det / (nline.A_ * nline.A_ + nline.B_ * nline.B_));
    return {Position(x_ref + nline.B_ * coef + center_x_,
                     y_ref - nline.A_ * coef + center_y_),
            Position(x_ref - nline.B_ * coef + center_x_,
                     y_ref + nline.A_ * coef + center_y_)};
  }
}
...
Circle::Intersect(const Circle &other) const {
  ...
  auto line = Line(-2 * xx, -2 * yy,
                   radius_ * radius_ - other.radius_ * other.radius_ + xx * xx +
                       yy * yy);
